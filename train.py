...
def train_cal(config, epoch, model, classifier, clothes_classifier, criterion_cla, criterion_pair, 
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids, cloth, _cloth, contour) in enumerate(trainloader):
        ...
        # Forward
        features, unclo_img, cont_img, clo_img = model(imgs) #features是model 跑出来的
        features_shuffle = features.clone() #克隆了一下、梯度会在原特征上传递
        ...
        pred_clothes = clothes_classifier(features.detach()) #注意这里detach了，说明梯度不要了，backbone接收不到反向传播，但是clothes_classifier处理后会有新的梯度
        _, preds = torch.max(outputs.data, 1)
        # Update the clothes discriminator
        clothes_loss = criterion_clothes(pred_clothes, clothes_ids) #训练服装分类器
        if epoch >= config.TRAIN.START_EPOCH_CC:
            ...
            optimizer_cc.step()
        # Update the backbone   更新固定好clothes_classifier权重之后，再跑之前的features，这一次backbone会在反向传播中更新
        ...
        if epoch >= config.TRAIN.START_EPOCH_ADV:
            ...
            loss = cla_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + config.PARA.RECON_RATIO * rec_loss  #这是0-25 epoch 之间训练的，重建loss+id loss
        optimizer.zero_grad()
        if config.TRAIN.AMP:
